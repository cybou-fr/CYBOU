...
pub fn CognitiveGraphContent(card: CardId) -> impl IntoView {
    ...
    view! {
        <div class="cognitive-graph-panel" style="display: flex; flex-direction: column; height: 100%; width: 100%; background: var(--bg-card); color: var(--text-main); font-family: system-ui, -apple-system, sans-serif; overflow: hidden;">
            // Header
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 8px;">
                        {move || signals.graph.get().map(|proj| {
                            proj.graph.nodes.into_iter().map(|node| {
                                ...
                                view! {
                                    <div
                                        style=move || format!("background: var(--bg-sunken); border: 1px solid {}; border-radius: 6px; padding: 10px; cursor: pointer; display: flex; flex-direction: column; gap: 6px; transition: border-color 0.15s ease;", if is_selected() { "var(--accent)" } else { "var(--line)" })
                                    ...
                                    >
                                        <div style="display: flex; align-items: center; justify-content: space-between;">
                                            ...
                                            <span style="font-size: 9px; color: var(--text-faint); font-family: monospace;">
                                        ...
                                        <div style="font-size: 9px; color: var(--text-faint); font-family: monospace; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                                            {node.id}
                                        </div>
                                    </div>
                                }
                                ...
                                <div style="width: 260px; border-left: 1px solid var(--line); padding: 12px; background: rgba(0,0,0,0.15); overflow-y: auto; display: flex; flex-direction: column; gap: 10px;">
                                    <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                                        <div>
                                            ...
                                            <div style="font-size: 10px; color: var(--accent-light); font-family: monospace; margin-top: 2px;">{node.id}</div>
                                        </div>
                                        <button
