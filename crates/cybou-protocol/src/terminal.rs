...
//! What a keystroke, a window size and an ending are, on the wire to an interactive terminal.
//!
//! Here rather than in `cybou-ptyd` because three parties have to agree about it: the browser that
//! draws the terminal, the gateway that carries the bytes, and the owner that allocates the
//! pseudoterminal. Leaving it in the owner would mean either a browser depending on a daemon or a
//! second copy of these enums in the web contracts, and two definitions of one wire is the drift
//! this repository keeps finding in itself.
//!
//! The bounds are here for the same reason. A gateway that refused a frame at a different size
//! from the owner would be a boundary with a gap in it exactly the width of the disagreement.
