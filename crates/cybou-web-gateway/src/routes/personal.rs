...
fn authenticated_uid(state: &GatewayState, headers: &HeaderMap) -> Result<u32, GatewayError> {
    state
        .session_for(headers)
...
mod tests {
    ...
    fn personal_hub_manages_mail_calendar_notes_contacts() {
        ...
        // Mail tests
        let mail = hub
            .get_mail(uid, None, Some(MailFolderKind::Inbox))
            .expect("mail");
        assert!(mail.accounts.is_empty());
        assert!(mail.messages.is_empty());

        let sent = hub.send_mail(
            uid,
            SendMailRequest {
        ...
        // Calendar tests
        let cal = hub.get_calendar(uid).expect("calendar");
        assert!(cal.events.is_empty());
        let event = hub.create_calendar_event(
            uid,
        ...
        assert!(event.is_ok());
        let cal_after = hub.get_calendar(uid).expect("calendar");
        assert_eq!(cal_after.events.len(), 1);

        // Notes tests
        let notes = hub.get_notes(uid).expect("notes");
        assert!(notes.notes.is_empty());
        let new_note = hub.create_note(
            uid,
            CreateNoteRequest {
        ...
        let notes_after = hub.get_notes(uid).expect("notes");
        assert_eq!(notes_after.notes.len(), 1);

        // Contacts tests
        let contacts = hub.get_contacts(uid).expect("contacts");
        assert!(contacts.contacts.is_empty());
        let new_cnt = hub.create_contact(
            uid,
        ...
        let contacts_after = hub.get_contacts(uid).expect("contacts");
