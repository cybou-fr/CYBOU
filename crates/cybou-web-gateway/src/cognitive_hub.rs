...
impl CognitiveHub {
    ...
    pub fn build_grounded_graph(
        ...
        for svc in services {
            ...
            meta.insert("active".to_owned(), (svc.state == cybou_protocol::system::ServiceState::Active).to_string());
        ...
        let daemon_relations = [
            ("service:cybou-web-gateway.service", "service:cybou-presenced.service", CognitiveEdgeType::Observes, "Web gateway observes Mind presence"),
            ("service:cybou-presenced.service", "service:cybou-eventd.service", CognitiveEdgeType::Observes, "Presence observes canonical Event1 stream"),
            ("service:cybou-actiond.service", "service:cybou-eventd.service", CognitiveEdgeType::Observes, "Action governor observes Event1 outcomes"),
            ("service:cybou-actiond.service", "service:cybou-executord.service", CognitiveEdgeType::Governs, "Action governor authorizes typed execution permits"),
            ("service:cybou-agentd.service", "path:/home/demo", CognitiveEdgeType::Governs, "Agent daemon confines workspaces under /home/demo"),
            ("service:cybou-host-filesd@demo.service", "path:/home/demo", CognitiveEdgeType::Governs, "Host files daemon serves bounded per-user home socket"),
        ];
            ...
            for node in &nodes {
                if node.id.starts_with("belief:") {
                    edges.push(CognitiveEdgeRecord {
                        ...
                        weight: 0.9,
                        description: "Epistemic belief derived from canonical event journal".to_owned(),
                    });
                    edge_seq += 1;
                }
            }
        ...
        if let Some(ref focus) = focus_node_id {
            ...
            let focus_matches: HashSet<String> = nodes
                .iter()
                .filter(|n| n.id.to_lowercase().contains(&focus_lower) || n.label.to_lowercase().contains(&focus_lower))
                .map(|n| n.id.clone())
                .collect();
            ...
            if !focus_matches.is_empty() {
                let relevant_edges: Vec<CognitiveEdgeRecord> = edges
                    .iter()
                    .filter(|e| focus_matches.contains(&e.source_id) || focus_matches.contains(&e.target_id))
                    .cloned()
                    .collect();
