...
- **Real-Time Journal & Live SSE Event Stream**: Gateway SSE event stream consumer (`/api/v1/events`) with topic filtering (Snapshots, System, Agents), real-time search with clear toggle, stream pause/resume controls (`● Live` / `⏸ Paused`), ring-buffer clearing with event counter, and deep event inspector drawer featuring formatted JSON and 1-click clipboard copy.
