...
- **Refresh boundary**: Browser navigation, refresh, or tab closure triggers the platform's unload confirmation while any editor instance holds a dirty or conflicted buffer. File contents are deliberately not copied into `localStorage`. Safe user-scoped draft recovery after a confirmed reload is not built yet, so SD1 is satisfied for layout/open tools but not yet for unsaved buffer contents.
