...
**Status: first fabric slice implemented.** `cybou-fabric` owns the frozen v1 D-Bus endpoint
registry and a strict CBOR envelope codec. Checked-in map/list byte streams are generated by the
predecessor Qt `FabricCodec`; the Debian gate recompiles that oracle and requires byte-identical
output before Rust tests run. The live Presence gateway now decodes through this shared crate.
Crypto, state paths, resilience, and the remaining owner-specific wire contracts are still pending.
