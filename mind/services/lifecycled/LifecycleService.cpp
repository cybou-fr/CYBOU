...
bool LifecycleService::load() {
    ...
    auto o=QJsonDocument::fromJson(f.readAll()).object(); m_mode=modeFrom(o["mode"].toString());
    ...
    if (m_hasRun && m_run.status==LifecycleRunStatus::Active) { m_mode=LifecycleMode::Recovering; return save(); }
    return true;
}
