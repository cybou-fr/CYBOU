...
(() => {
  ...
  const translations = {
    en: {
      ...
      faq_a5: 'Cybou ships no language model, no agent platform, no tool broker, no privileged action executor, and no autonomous security control plane. The model broker exists as a faculty with no worker behind it. What comes next is decided rather than sketched: an agent runs inside a kernel-enforced capsule, is autonomous within it, and produces an action proposal at its boundary. These are planned boundaries and are never described as done.',
      ...
      rm4_title: 'Governed action, then agent capsules',
      ...
      rm4_sub: 'Executor · Independent re-observation · Capsules · Model gateway',
      ...
      row_m913_cap: 'Agent capsules, learning, agents and security',
      row_m913_bound: 'The action boundary is built — typed proposals, criticism, standing policy, authorization — and nothing behind it can execute. The agent platform is decided and unbuilt: a capsule the kernel enforces, autonomy inside it, a proposal at its boundary, and a model lease instead of a provider credential.',
    ...
    fr: {
      ...
      faq_a5: 'Cybou ne livre aucun modèle de langage, aucune plateforme d’agents, aucun courtier d’outils, aucun exécuteur d’action privilégiée et aucun plan de contrôle de sécurité autonome. Le courtier de modèles existe comme faculté sans worker derrière lui. La suite est décidée et non esquissée : un agent s’exécute dans une capsule imposée par le noyau, y est autonome, et produit une proposition d’action à sa frontière. Ce sont des frontières prévues, jamais décrites comme faites.',
      ...
      rm4_title: 'Action gouvernée, puis capsules d’agents',
      ...
      rm4_sub: 'Exécuteur · Réobservation indépendante · Capsules · Passerelle de modèles',
      rm_complete: 'Terminé',
      ...
      row_m913_cap: 'Capsules d’agents, apprentissage, agents et sécurité',
      row_m913_bound: 'La frontière d’action est construite — propositions typées, critique, politique permanente, autorisation — et rien derrière elle ne peut exécuter. La plateforme d’agents est décidée et non construite : une capsule imposée par le noyau, l’autonomie à l’intérieur, une proposition à sa frontière, et un bail de modèle au lieu d’un identifiant de fournisseur.',
    ...
    ru: {
      ...
      faq_a5: 'Cybou не поставляет ни языковой модели, ни платформы агентов, ни брокера инструментов, ни исполнителя привилегированных действий, ни автономного плана управления безопасностью. Брокер моделей существует как факультет, за которым нет worker. Дальнейшее решено, а не намечено: агент работает внутри капсулы, которую держит ядро, автономен внутри неё и формирует предложение действия на её границе. Это запланированные границы, и они никогда не описываются как готовые.',
      ...
      rm4_title: 'Управляемое действие, затем капсулы агентов',
      ...
      rm4_sub: 'Исполнитель · Независимый переосмотр · Капсулы · Шлюз моделей',
      rm_complete: 'Готово',
      ...
      row_m913_cap: 'Капсулы агентов, обучение, агенты и безопасность',
      row_m913_bound: 'Граница действия построена — типизированные предложения, критика, постоянная политика, разрешение — и ничто за ней не может выполнять. Платформа агентов решена и не построена: капсула, которую держит ядро, автономия внутри неё, предложение на её границе и аренда модели вместо учётных данных провайдера.',
